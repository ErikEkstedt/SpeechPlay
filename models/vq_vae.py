...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self._residual_stack = ResidualStack(in_channels=filters[-1],
                                             num_hiddens=filters[-1],
...
if __name__ == "__main__":
    ...
    spec = torch.from_numpy(spec).unsqueeze(0).unsqueeze(0).to(device)

    loss, out, perplexity = model(spec)  # out.shape 1,1, 96, 162
